...
impl<'c> QueryBuilder<'c> {
    ...
    pub async fn json<T: DeserializeOwned>(self) -> Result<T, LlmError> {
        let raw = self.run().await?;
...
impl<'c> QueryBuilderWithTokens<'c> {
    ...
    pub async fn json<T: DeserializeOwned>(self) -> Result<WithTokenUsage<T>, LlmError> {
        let with_tokens = self.run().await?;
        let result =
...
impl<'c> CacheBuilder<'c> {
    ...
    pub async fn build(self) -> Result<CacheResult, LlmError> {
        self.client
            .execute_cache(&self.content, &self.attachments)
            .await
    }
}
