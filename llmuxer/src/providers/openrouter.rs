...
pub type OpenRouterProvider = OpenRouterClient<reqwest::blocking::Client>;
#[cfg(feature = "async")]
pub type OpenRouterProvider = OpenRouterClient<reqwest::Client>;

pub struct OpenRouterClient<C> {
    api_key: String,
    base_url: String,
    model: String,
    instruction: String,
...
#[cfg(not(feature = "async"))]
impl OpenRouterClient<reqwest::blocking::Client> {
    pub(crate) fn new(config: ClientConfig) -> Result<Self, LlmError> {
        let base_url = match config.base_url {
            Some(url) => url,
            None => "https://openrouter.ai/api".to_string(),
        };
        Ok(Self {
    ...
    fn send_request(&self, body: Value) -> Result<(Value, String), LlmError> {
        ...
        let resp = self
            ...
            .header("HTTP-Referer", "https://github.com/opencode-ai/opencode")
...
#[cfg(not(feature = "async"))]
impl LlmClient for OpenRouterClient<reqwest::blocking::Client> {
    ...
    fn execute_query_with_tokens(
        ...
        let (parsed, raw) = self.send_request(body)?;
        let token_usage = token_extraction::extract_openrouter(&parsed);
...
#[cfg(feature = "async")]
impl OpenRouterClient<reqwest::Client> {
    pub(crate) fn new(config: ClientConfig) -> Result<Self, LlmError> {
        let base_url = match config.base_url {
            Some(url) => url,
            None => "https://openrouter.ai/api".to_string(),
...
#[cfg(feature = "async")]
impl LlmClient for OpenRouterClient<reqwest::Client> {
    fn query(&self, prompt: &str) -> crate::traits::QueryBuilder<'_> {
        crate::traits::QueryBuilder::new(self, prompt)
    }
...
mod tests {
    ...
    #[test]
    fn reasoning_effort_in_nested_object() {
